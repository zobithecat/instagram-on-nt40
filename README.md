...
- [x] **마일스톤 8 — 한글 렌더링 + 소켓 타임아웃**: `core/font`가 KS X 1001(1987
      완성형) 한글 2350자 비트맵(18x18, `core/font_hangul_data.h`, `tools/
      gen_hangul_font.py`로 실제 한글 폰트에서 래스터라이즈해 생성 — GDI/트루타입에
      기대지 않고 `ui/feed.c`의 "순수 C, `make preview`와 NT4가 완전히 동일한
      프레임버퍼" 원칙 유지)를 지원. UTF-8 3바이트 시퀀스를 디코드해 테이블에 있으면
      실제 글리프, 없으면(다른 스크립트·이모지·희귀 한글) 코드포인트 단위로 한 칸
      빈칸 처리. 14px에서는 저/져, 처/쳐 같은 유사 모음쌍이 뭉개져 18px로 확정
      (직접 비교 렌더링으로 확인). 생성 중 행 비트맵이 16비트(`unsigned short`)를
      넘는 값(18비트 필요)이 잘리는 실버그를 컴파일 경고로 발견해 `unsigned int`로
      수정. 실물 NT4 검증 중 GUI 부팅이 아예 안 뜨는 것처럼 보이는 문제도 조사 —
      원인은 두 가지: (1) `pal/net_win32.c`의 connect/send/recv에 **타임아웃이
      전혀 없어** 응답이 없으면 창이 뜨기도 전에 영원히 멈출 수 있었음 (non-blocking
      connect+select 및 SO_RCVTIMEO/SNDTIMEO로 15초 타임아웃 추가), (2) 이번 건은
...
- [x] **마일스톤 9 — 캡션 "더보기" 아코디언**: 실물 NT4에서 여러 줄짜리 실제 한글
      ...
      잘리고 다음 카드와 전혀 안 겹치는 것 확인. 클릭으로 펼치기/접기도 실물
      NT4에서 정상 동작 확인 완료
