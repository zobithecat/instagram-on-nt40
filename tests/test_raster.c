...
#include "../core/font.h"
#include "../core/font_hangul_data.h" /* HANGUL_W/H -- generated data, not part of font.h's public API */
#include "../core/json.h"
#include "../core/http.h"
...
/* KS X 1001 Hangul glyphs (core/font_hangul_data.h, generated by
 * tools/gen_hangul_font.py). U+AC00 "가" is HANGUL_CODEPOINTS[0] -- the first
 * syllable in the table -- and its UTF-8 encoding is 0xEA 0xB0 0x80. */
static void test_hangul(void) {
    CHECK(font_text_width("\xea\xb0\x80", 1) == HANGUL_W);

    Surface s;
    surface_alloc(&s, HANGUL_W + 8, HANGUL_H + 4);
    surface_fill(&s, 0);
    font_draw(&s, 0, 0, "\xea\xb0\x80", ras_rgb(255, 255, 255)); /* "가" */
    int any_set = 0;
    for (int y = 0; y < HANGUL_H; y++)
        for (int x = 0; x < HANGUL_W; x++)
            if (at(&s, x, y) != 0) any_set = 1;
    CHECK(any_set); /* a real glyph was rasterized, not left blank */
    CHECK_EQ(at(&s, HANGUL_W + 4, 2), 0); /* nothing past the single glyph's cell */

    /* mixed ASCII + Hangul: width sums both, no crash */
    int w_ascii = font_text_width("A", 1);
    int w_mixed = font_text_width("A\xea\xb0\x80", 1); /* "A" + "가" */
    CHECK(w_mixed > w_ascii);
    surface_fill(&s, 0);
    font_draw(&s, 0, 0, "A\xea\xb0\x80", ras_rgb(255, 255, 255));

    /* a valid 3-byte UTF-8 sequence outside the KS X 1001 subset (U+2603
     * SNOWMAN, nowhere near the Hangul block) advances blank, not garbage */
    surface_fill(&s, 0);
    font_draw(&s, 0, 0, "\xe2\x98\x83", ras_rgb(255, 255, 255));

    /* truncated/malformed sequences at the end of the string must not read
     * past the NUL terminator (ASan would catch an OOB read) */
    font_draw(&s, 0, 0, "x\xe2\x98", ras_rgb(255, 255, 255));  /* lead + 1 of 2 continuation bytes */
    font_draw(&s, 0, 0, "x\xe2", ras_rgb(255, 255, 255));      /* lead byte only */
    font_draw(&s, 0, 0, "x\xc2", ras_rgb(255, 255, 255));      /* 2-byte lead, no continuation */
